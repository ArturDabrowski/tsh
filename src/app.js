...
export class App {
  initializeApp() {
    ...
    $('.events-container').on(
      'click',
      '.timeline-marker, .timeline-item',
      function() {
        $(this)
          .parent()
          .find('.timeline-item, .timeline-marker')
    ...
    fetch(`https://api.github.com/users/${userName}`)
      .then(userResponse => {
        ...
        $('.profile-container').prepend(
          '<h2 class="subtitle is-4">No such profile</h2>'
        )
  ...
  update_history(events) {
    ...
    $('.timeline').append(
      events
        .filter(event => eventTypes.some(eventType => eventType === event.type))
